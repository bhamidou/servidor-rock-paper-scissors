...
class ControllerPartida extends Controller
{
    ...
    public function store(Request $req)
    {
        ...
        $validator = $this->validatorPartida($req);
        ...
        if (count(DB::select('select * from partida where id_ronda = ?', [$idronda])) < 5) {
            ...
            if ($select[0]->victorias >= 3 || $select[1]->victorias >= 3) {
                ...
                return response()->json(["msg" => $rtnMsg, "ronda" => "terminada"], 202);
        ...
        $validator = Validator::make($req->all(), [
            ...
            'id_user_1' => [
                ...
                Rule::exists('ronda', 'id_user_1')->where(function (Builder $query) use ($req) {
                    return $query->where('id_user_1', $req->input('id_user_1'))->where('id', $req->get('id_ronda'));
                }),
                'different:id_user_2'
            ],
            ...
            'id_user_2' => [
                ...
                Rule::exists('ronda', 'id_user_2')->where(function (Builder $query) use ($req) {
                    return $query->where('id_user_2', $req->input('id_user_2'))->where('id', $req->get('id_ronda'));
                }),
                'different:id_user_1'
            ],
